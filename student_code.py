...
class KnowledgeBase(object):
    ...
    def helper(self,fact_rule,string,depth):
        for each in fact_rule.supported_by:
            string+="\t"*depth+ "SUPPORTED BY\n"
            string += "\t"*(depth+1)+"fact：" + str(each[0].statement)
            if each[0].asserted:
                string += " ASSERTED\n"
            string += "\t"*depth+"rule: ("

            for i in range(len(each[1].lhs)):
                string += str(each[1].lhs[i])
            string.join(",")
            string+=")"
            string+=" -> " + str(each[1].rhs)
            if each[1].asserted:
                string += " ASSERTED\n"
                depth-1
                return (string,depth)
            else:
                self.helper(each[1],string,depth+1)
